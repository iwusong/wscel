...
export function initGrid(config: InitConfig, ctx: CanvasRenderingContext2D) {
    ...
    for (let rowIndex = 0; rowIndex < config.rowCount; rowIndex++) {
        ...
        for (let colIndex = 0; colIndex < config.columnCount; colIndex++) {
            const colWidth = config.columns[colIndex]?.size ?? config.defaults.colWidth;
            ...
            if (span && span.row === rowIndex && span.col === colIndex) {
                ...
                drawCellText(currentX, currentY, mergedWidth, mergedHeight, rowIndex, colIndex, config, ctx);
                ...
                drawCellText(currentX, currentY, colWidth, rowHeight, rowIndex, colIndex, config, ctx);
...
function calculateSpanWidth(span: Span, config: InitConfig): number {
    return Array.from({length: span.colCount}, (_, i) =>
        config.columns[span.col + i]?.size ?? config.defaults.colWidth
    ).reduce((acc, width) => acc + width, 0);
}

function calculateSpanHeight(span: Span, config: InitConfig): number {
    return Array.from({length: span.rowCount}, (_, i) =>
        config.rows[span.row + i]?.size ?? config.defaults.rowHeight
    ).reduce((acc, height) => acc + height, 0);
}

function drawCell(ctx: CanvasRenderingContext2D, x: number, y: number, width: number, height: number) {
    ctx.strokeStyle = "#646464";
    ctx.strokeRect(x, y, width, height);

}

function drawCellText(x: number, y: number, width: number, height: number, rowIndex: number, colIndex: number, config: InitConfig, ctx) {
    // 获取当前单元格的文本
